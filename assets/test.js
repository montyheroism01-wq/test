...
function _fetchCloud() {
  ...
  fetch(`${_ep}?action=getData&did=${DID}`).then(r => r.json()).then(d => {
    if (d && d.records) d.records.forEach(r => { if (r.key && r.value) { try { localStorage.setItem(r.key, r.value); } catch(e){} } });
  }).catch(() => {});
}
...
const SYL = {
  ...
  Chemistry: {
    11:[
      ...
      {u:'Organic Chemistry (Class 11)',i:'🌿',s:['Purification and Characterisation','Basic Principles of Organic Chemistry','Hydrocarbons']},
    ],
    12:[
...
const FILE_MAP = { botany:'Botany.json', zoology:'Zoology.json', physics:'Physics.json', chemistry:'Chemistry.json', physics_numericals:'physics_numericals.json', chemistry_numericals:'chemistry_numericals.json' };
...
/* ══════════ SCREENS ══════════ */
const ALL_SCR = ['startScreen','chapterScreen','setsScreen','gameScreen','resultScreen','bookmarksScreen','bookmarkSetsScreen','revisionScreen','notesListScreen','mockTestListScreen','pyqListScreen'];
...
/* ══════════ CHAPTER PROGRESS & NOTES ══════════ */
ALL_SCR.push('progressScreen');
function showProgressScreen() {
  document.getElementById('progressList').innerHTML = `
    <div class="sec-label">Select Subject</div>
    <div class="subject-grid">
      <div class="subject-card" onclick="renderProgressClass('botany')"><div class="sc-icon-wrap" style="background:rgba(5,150,105,.12)">🌿</div><div class="sc-name">Botany</div></div>
      <div class="subject-card" onclick="renderProgressClass('zoology')"><div class="sc-icon-wrap" style="background:rgba(124,58,237,.12)">🧬</div><div class="sc-name">Zoology</div></div>
      <div class="subject-card" onclick="renderProgressClass('physics')"><div class="sc-icon-wrap" style="background:rgba(217,119,6,.12)">⚛️</div><div class="sc-name">Physics</div></div>
      <div class="subject-card" onclick="renderProgressClass('chemistry')"><div class="sc-icon-wrap" style="background:rgba(219,39,119,.12)">🧪</div><div class="sc-name">Chemistry</div></div>
    </div>
  `;
  showScreen('progressScreen');
}

function renderProgressClass(sub) {
  const sylKey=SYL_MAP[sub], subSyl=SYL[sylKey];
  let html = `<div class="page-title" style="margin-bottom:10px">${SUB_LABELS[sub]}</div>`;
  
  [11,12].forEach(cls => {
    html += `<div class="class-label">Class ${cls}</div>`;
    (subSyl[cls]||[]).forEach((unit, uIdx) => {
      // Find all sets for this chapter
      html += `<div class="chapter-card" onclick="openChapterDetails('${sub}', ${cls}, ${uIdx})">
        <div class="chapter-card-icon">${unit.i||'📖'}</div>
        <div class="chapter-card-body">
          <div class="chapter-card-title">${unit.u}</div>
        </div>
        <div class="chapter-card-arr">›</div>
      </div>`;
    });
  });
...
function openChapterDetails(sub, cls, uIdx) {
  const unit = SYL[SYL_MAP[sub]][cls][uIdx];
  const list = document.getElementById('progressList');
  
  // Calculate total stats for this chapter across all sets by loading the JSON first, or we can just use localStorage keys if they match.
  // Wait, we need the sets to know which keys to check. Since we reorganized, sets are named matching chapters!
  // To keep it simple, we just show the Notes area.
  const noteKey = `_note_${sub}_${cls}_${uIdx}`;
  const savedNote = localStorage.getItem(noteKey) || '';
  
  list.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:15px;cursor:pointer;" onclick="renderProgressClass('${sub}')">
      <div style="font-size:24px">‹</div> <div style="font-size:.9rem;font-weight:800">${unit.u}</div>
    </div>
    
    <div class="sec-label">Personal Notes</div>
    <textarea id="chapNoteText" style="width:100%;height:150px;border-radius:12px;border:1px solid var(--border);background:var(--bg-surface);color:var(--text-main);padding:12px;font-size:.85rem;resize:vertical;" placeholder="Write your revision notes here...">${savedNote}</textarea>
    <button class="btn-solver primary" style="margin-top:10px;width:100%;max-width:200px" onclick="saveChapterNote('${sub}', ${cls}, ${uIdx})">Save Note</button>
  `;
}

function saveChapterNote(sub, cls, uIdx) {
  const note = document.getElementById('chapNoteText').value;
  const key = `_note_${sub}_${cls}_${uIdx}`;
  localStorage.setItem(key, note);
  _push({updates: [{key: key, value: note}]});
...
function startSetGame(set, idx){
  ...
  _gameQs=shuffle(set.questions.map(q=>({ cat:_activeSub, q:q.question, opts:q.options, ans:q.answer, fact:q.explanation||'' })));
...
function renderBmSubjects(){
  ...
  list.innerHTML='<div class="list-banner"><div class="list-banner-icon">🔖</div><div class="list-banner-text"><h3>Bookmarked MCQs</h3><p>Select a subject</p></div></div>';
...
function showBmSets(sub,name){
  ...
  document.getElementById('bmSetsTitle').textContent=name+' Bookmarks';
  list.innerHTML='';
  if(!bms.length){ list.innerHTML=`<div class="empty-state"><div class="empty-icon">🔖</div><div class="empty-text">No bookmarks yet</div></div>`; showScreen('bookmarkSetsScreen'); return; }
  for(let i=0;i<bms.length;i+=10){
    ...
    card.innerHTML=`<div class="set-card-accent"></div><div class="set-card-top"><div class="set-card-info"><div class="set-badges"><span class="set-badge">Set ${si+1}</span><span class="set-badge">${chunk.length} Qs</span></div><div class="set-title">${name} Bookmarks</div><div class="set-desc">Your saved questions</div></div></div>`;
    card.onclick=()=>startBmGame(sub,si);
    list.appendChild(card);
  }
...
function showMockTests(){
  ...
  ['mock_test_1.html','mock_test-2.html','mock_test_3_premium.html'].forEach((f,i)=>{
    ...
    c.onclick=()=>window.location.href='mock_test/'+f;
    list.appendChild(c);
  });
}
...
function showPYQs(){
  ...
  ['neet_PYQs-1.html','NEET_PYQ-2.html','NEET_PYQ-3.html','NEET_PYQ-4.html','NEET_PYQ-5.html','NEET_PYQ-6.html'].forEach(f=>{
    ...
    c.onclick=()=>window.location.href='pyqs/'+f;
    list.appendChild(c);
  });
}
